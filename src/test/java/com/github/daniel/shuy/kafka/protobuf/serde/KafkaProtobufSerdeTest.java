...
public class KafkaProtobufSerdeTest {
  ...
  private <T extends MessageLite> void stream(T input, T output, Parser<T> parser) {
    ...
    Predicate<T, T> predicate =
        (key, value) -> Objects.equals(key, input) && Objects.equals(value, input);
    KStream<T, T>[] streams =
        builder
    ...
    ConsumerRecordFactory<T, T> factory =
        new ConsumerRecordFactory<>(inputTopic, serializer, serializer);
    testDriver.pipeInput(factory.create(input, input));

    Deserializer<T> deserializer = serde.deserializer();
    ProducerRecord<Object, String> errorRecord =
        testDriver.readOutput(errorTopic, null, new StringDeserializer());
    if (errorRecord != null) {
      throw new AssertionError(errorRecord.value());
    }

    ProducerRecord<T, T> outputRecord =
        testDriver.readOutput(outputTopic, deserializer, deserializer);
